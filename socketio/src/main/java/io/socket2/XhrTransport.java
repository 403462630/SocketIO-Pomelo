...
class XhrTransport implements IOTransport {
    ...
    private class PollThread extends Thread {
        ...
        public void run() {
            ...
            //	try {
            //		String line;
            //		URL url = new URL(io.socket.XhrTransport.this.url.toString() + "?t="
            //				+ System.currentTimeMillis());
            //		urlConnection = (HttpURLConnection) url.openConnection();
            //		SSLContext context = io.socket.IOConnection.getSslContext();
            ...
            //					line = io.socket.IOConnection.FRAME_DELIMITER + junk.length()
            //							+ io.socket.IOConnection.FRAME_DELIMITER + junk;
            //					output.write(line.getBytes(CHARSET));
            //					iter.remove();
            //				}
            //			}
    ...
    public static IOTransport create(URL url, IOConnection connection) {
        try {
            URL xhrUrl = new URL(url.toString() + IOConnection.SOCKET_IO_1 + TRANSPORT_NAME + "/"
                                         + connection.getSessionId());
            return new XhrTransport(xhrUrl, connection);
        } catch (MalformedURLException e) {
            throw new RuntimeException(
                    "Malformed Internal url. This should never happen. Please report a bug.", e);
        }
    }
